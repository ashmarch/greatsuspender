...
const tgs = (function () {
  ...
  function initAsPromised() {
    return new Promise(async function (resolve) {
      ...
      if (!chrome.extension.inIncognitoContext) {
        ...
        const contextMenus = gsStorage.getOption(gsStorage.ADD_CONTEXT);
  ...
  async function handleSuspendedTabFocusGained(focusedTab) {
    ...
    const autoUnsuspend = gsStorage.getOption(gsStorage.UNSUSPEND_ON_FOCUS);
    if (autoUnsuspend) {
      if (navigator.onLine) {
  ...
  function checkForNotices() {
    ...
    const lastShownNoticeVersion = gsStorage.fetchNoticeVersion();
